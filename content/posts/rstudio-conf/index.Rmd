...
Now we have our recipe and our model, we can create our "workflow", which packages up our the preprocessing steps and model. Using workflows, we don't need to go through the prep() and juice() steps we went through earlier, we can simply define our recipe and specify our model as we do below.
...
Nonetheless, being able to prep() our data can be useful for when our preprocessing requires training that we might want to save, and being able to juice() our data can be useful for being able to inspect our processed data as we did earlier.
...
### Model evaluation
How does our model perform on our training set? Let's find out using metrics from the {yardstick} package. We'll use three metrics: Root Mean Squared Error (RMSE), $R^2$, and the concordance correlation coefficient (ccc).

First we'll set our three metrics, then we'll generate predictions, back-transform them to the original scale, and compare those back-transformed predictions to the true values.
...
Looks great! But of course, this is all in-sample. Perhaps we want to know what kind of out-of-sample performance we can expect from our model using cross-validation. {rsample} also makes that easy, so let's create 10-fold cross-validation sets for evaluating our training set models using vfold_cv(), which defaults to creating 10 folds.
...
### Model tuning
Recall that we set our regularization penalty to 0.001 and we chose to use L1 regularization. Both those decisions were relatively arbitrary. Let's use the {tune} package to explore other penalty values and regularization mixtures using cross validation performance to decide what values these parameters should be.

We'll start by defining a new model, ames_mixture, which will not take specific specific values for penalty and mixture and will instead leave these as variables to be tuned.
...
{tune} provides useful defaults for searching parameter spaces, for example, creating grids for the "penalty" parameter in log-10 space. We can simply specify the parameters, pass these to grid_regular(), and specify that we want 5 levels of penalization and 5 levels of mixture.
...
First, lets register the 6 cores on my laptop.
...
Now we're going to create our tuning object, which will take our recipe, our model, our resamples, and our metrics, to fit our 25 models over 10 resamples and compute our performance metrics, then stop our parallelization.
...
Now let's select our best grid candidate and finalize our model.
...
ames_mixture_final <- finalize_model(ames_mixture, best_mixture)
```

And finally, let's create our workflow using this finalized model and fit our model

```{r mixtureworkflow}

ames_mixture_wfl <- workflow() %>% 
  add_recipe(ames_rec) %>% 
  add_model(ames_mixture_final)

ames_mixture_fit <- fit(ames_mixture_wfl, ames_train)
```

And we're done! We now have a fitted, tuned, regularized mixtured model (allbeit, the mixture is 100% L2 regularization, but we got there via tuning!)

*Finally* we get to the fun part. What variables turned out to be the most important in predicting sale price?

```{r varimportance}
tidy_coefs <- ames_mixture_fit$fit$fit %>% 
  broom::tidy() %>% 
