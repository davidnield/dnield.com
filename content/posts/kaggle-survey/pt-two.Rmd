...
tsdata %>% 
  ...
  mutate(income_dbl = case_when(
    income == '50,000 - 60,000' ~ mean(50000, 60000),
    income == '60,000 - 70,000' ~ mean(60000, 70000),
    income == '70,000 - 80,000' ~ mean(70000, 80000),
    income == '80,000 - 90,000' ~ mean(80000, 90000),
    income == '90,000 - 100,000' ~ mean(90000, 10000),
    income == '100,000 - 125,000' ~ mean(100000, 125000),
    income == '125,000 - 150,000' ~ mean(125000, 150000)
  )) %>% 
  filter(!is.na(job_title)) %>% 
...
tsdata %>% 
  ...
  mutate(language = str_extract(language, pattern = "(?<=language_).*"),
         language = case_when(language == 'python' ~ 'Python',
                              ...
                              language == 'R' ~ 'R')) %>% 
  ggplot(aes(x = year, y = prop, group = language, label = language, color = language)) +
  geom_line() +
  geom_label_repel(min.segment.length = 1, direction = 'y', box.padding = 0.1, seed = 42) +
  scale_y_continuous(label = scales::percent) +
  labs(x = "Year",
       y = "Percent") +
  theme_ed +
